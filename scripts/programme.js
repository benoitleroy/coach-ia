...
export function blocCourant(now = new Date()) {
  ...
  return {
    ...
    decharge: bloc ? semaineDansBloc === bloc.semaines : false,
    avantDebut: semaineDansCycle < 1, termine: semaineDansCycle > total,
  };
}

// Bloc à planifier = celui de la semaine PROCHAINE (le coach écrit la semaine suivante)
export function blocAPlanifier(now = new Date()) {
  return blocCourant(new Date(now.getTime() + 7 * DAY));
}
