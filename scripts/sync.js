...
import { avecEnfants } from "./garde.js";
...
// ─── HISTORIQUE DES SÉANCES (page Programme, consultable à la box) ───────────
function writeSeances(activities, extras) {
  const since = Date.now() - 180 * 86400000;
  const seances = activities
    .filter(a => new Date(a.start_date).getTime() >= since)
    .sort((a, b) => new Date(b.start_date) - new Date(a.start_date))
    .map(a => ({
      id: a.id,
  ...
  fs.writeFileSync(path.join(__dirname, "../js/seances-data.js"),
    `// AUTO-GENERATED by scripts/sync.js — ne pas éditer.\nwindow.SEANCES = ${JSON.stringify(seances, null, 1)};\n`);
}
...
`;
  ...
  try {
    ...
    try {
      ...
      // Garde des enfants sur 8 semaines → contraintes du matin visibles dans l'app
      const garde = {};
      for (let i = -7; i < 56; i++) {
        const d = new Date(Date.now() + i * 86400000);
        garde[`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`] = avecEnfants(d);
      }
      fs.writeFileSync(path.join(__dirname, "../js/garde-data.js"),
        `// AUTO-GENERATED by scripts/sync.js — ne pas éditer.\nwindow.GARDE = ${JSON.stringify(garde)};\n`);
      ...
      fs.writeFileSync(path.join(__dirname, "../js/mouvements-data.js"),
        `// AUTO-GENERATED by scripts/sync.js — ne pas éditer.\nwindow.MOUVEMENTS = ${JSON.stringify(MOUVEMENTS.map(m => ({ en: m.en, fr: m.fr, alias: m.alias || [], desc: m.desc || "", schema: m.schema || null, cat: m.cat })))};\n`);
      fs.writeFileSync(path.join(__dirname, "../js/nst-data.js"),
        `// AUTO-GENERATED by scripts/sync.js — ne pas éditer.\nwindow.NST = ${JSON.stringify(loadNST(), null, 1)};\n`);
      writeSeances(activities, extras);
